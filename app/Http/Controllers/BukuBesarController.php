...
class BukuBesarController extends Controller
{
    public function index(Request $request)
    {
                ...
                case 'periode':
                    ...
                    switch ($request->periode) {
                        ...
                        case '1-minggu-terakhir':
                            $jurnal_umum = JurnalUmum::where('akun_id', $akun->id)
                                ->whereBetween('tanggal', [date('Y-m-d', strtotime('-7 day')), date('Y-m-d')])
                                ->orderBy('tanggal', $request->tanggal == 1 ? 'desc' : 'asc')
                                ->get();
                            $jurnal_penyesuaian = JurnalPenyesuaian::where('akun_id', $akun->id)
                                ->whereBetween('tanggal', [date('Y-m-d', strtotime('-7 day')), date('Y-m-d')])
                                ->orderBy('tanggal', $request->tanggal == 1 ? 'desc' : 'asc')
                                ->get();
                            break;
        ...
        $jurnal = $jurnal->sortBy(function($item) use ($request) {
            return $request->tanggal == 1 ? $item['tanggal'] : -strtotime($item['tanggal']);
        });
